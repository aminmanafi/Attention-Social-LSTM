...
from temp import get_attention_weights
...
def main():
    ...
    for iteration in range(sample_args.iteration):
        ...
        for batch in range(dataloader.num_batches):
            ...
            prepared_data = prepare_data_for_attentions(x_seq, lookup_seq, target_id)
            size = np.shape(prepared_data)
            attention_weights = np.zeros((size[0],size[1]))
            
            for i in range(1,len(prepared_data)):
                num_nodes = len(PedsList_seq[i])
                nodeIDs = [int(nodeID) for nodeID in PedsList_seq[i]]
                list_of_nodes = [lookup_seq[x] for x in nodeIDs]
                #del list_of_nodes[lookup_seq[target_id]]
                list_of_nodes.remove(lookup_seq[target_id])
                
                
                if len(list_of_nodes)>0:
                    #attention_weights[i][list_of_nodes] = get_attention_weights(prepared_data[i][list_of_nodes])
                    attention_weights[i] = get_attention_weights(prepared_data[i])
            attention_weights = np.insert(attention_weights,lookup_seq[target_id],0.0,1)
            ...
            if sample_args.method == 3: #vanilla lstm
                ...
                # Extract the observed part of the trajectories
                obs_traj, obs_PedsList_seq, obs_grid, obs_att_weights = x_seq[:sample_args.obs_length], PedsList_seq[:sample_args.obs_length], grid_seq[:sample_args.obs_length], attention_weights[:sample_args.obs_length]
                ret_x_seq = sample(obs_traj, obs_PedsList_seq, sample_args, net, x_seq, PedsList_seq, saved_args, dataset_data, dataloader, lookup_seq, numPedsList_seq, sample_args.gru, obs_att_weights, obs_grid)
            ...
            total_error += get_mean_error(ret_x_seq[sample_args.obs_length:].data, orig_x_seq[sample_args.obs_length:].data, PedsList_seq[sample_args.obs_length:], PedsList_seq[sample_args.obs_length:], sample_args.use_cuda, lookup_seq)
            final_error += get_final_error(ret_x_seq[sample_args.obs_length:].data, orig_x_seq[sample_args.obs_length:].data, PedsList_seq[sample_args.obs_length:], PedsList_seq[sample_args.obs_length:], lookup_seq)
...
def sample(x_seq, Pedlist, args, net, true_x_seq, true_Pedlist, saved_args, dimensions, dataloader, look_up, num_pedlist, is_gru, attention_weights, grid = None):
    '''
    The sample function
    params:
    ...
    with torch.no_grad():
        ...
        for tstep in range(args.obs_length-1):
            if grid is None: #vanilla lstm
               # Do a forward prop
                ...
                # Do a forward prop
                out_obs, hidden_states, cell_states = net(x_seq[tstep].view(1, numx_seq, 2), [grid[tstep]], hidden_states, cell_states, [Pedlist[tstep]], [num_pedlist[tstep]], dataloader, look_up, attention_weights)
        ...
        for tstep in range(args.obs_length-1, args.pred_length + args.obs_length-1):
            ...
            if grid is None: #vanilla lstm
                ...
                outputs, hidden_states, cell_states = net(ret_x_seq[tstep].view(1, numx_seq, 2), [prev_grid], hidden_states, cell_states, [true_Pedlist[tstep]], [num_pedlist[tstep]], dataloader, look_up, attention_weights)
...
def prepare_data_for_attentions(data,lookup_seq,target_id):
    keys = list(lookup_seq.keys())
    flag = 0
    
    res = np.zeros((len(data),len(keys)-1,10))
    
    for i in range(len(data)):
        flag = 0
        for j in range(len(keys)):
            if j == lookup_seq[target_id]:
                res[i,:,:2] = data[i,lookup_seq[target_id],:]
                if i >0:    
                    res[i,:,2] = (data[i,j,0] - data[i-1,j,0]) / 10
                    res[i,:,3] = (data[i,j,1] - data[i-1,j,1]) / 10
                res[i,:,4] = 1
                flag = 1
            else:
                if flag:
                    res[i,j-1,5:7] = data[i,j,:]
                    if i >0:
                        res[i,j-1,7] = (data[i,j,0] - data[i-1,j,0]) / 10
                        res[i,j-1,8] = (data[i,j,1] - data[i-1,j,1]) / 10
                else:
                    res[i,j,5:7] = data[i,j,:]
                    if i >0:
                        res[i,j,7] = (data[i,j,0] - data[i-1,j,0]) / 10
                        res[i,j,8] = (data[i,j,1] - data[i-1,j,1]) / 10
                res[i,j-1,9] = 1
    return res
    
    
    
    # res = np.zeros((len(data),len(keys),10))
