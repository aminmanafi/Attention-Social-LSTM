...
class SocialModel(nn.Module):
    ...
    def __init__(self, args, infer=False):
        ...
        #######################################################################
        # Linear layer to embed the attention weights
        
        self.attention_weights_layer = nn.Linear(self.input_size - 1, self.embedding_size)
    ...
    def forward(self, *args):
        ...
        for framenum,frame in enumerate(input_data):
            ...
            current_weights = attention_weights[framenum][list_of_nodes]
            current_weights = np.reshape(current_weights, (-1,1))
            current_weights = torch.tensor(current_weights)
            ...
            # Embed the attention weights
            attention_weights_embedded = self.dropout(self.relu(self.attention_weights_layer(current_weights.float())))
            
            ###################################################################

            # Concat input
            concat_embedded = torch.cat((input_embedded, tensor_embedded,attention_weights_embedded), 1)
